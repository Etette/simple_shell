 ...
 * @b: buffer structure
 * @enpv: environ structure
 * @retrn_value: value of previous command
 */
void variable_expansion(buffer *b, env_t *envp, int retrn_value)
{
	...
	offset = index = 0;
	while ((index = _strstr_int(b->buf + b->bp + offset, "$")) >= 0 && index < _strpbrk_int(b->buf + b->bp + offset, "\n;&|"))
	{
		index += offset;
		offset = index + 1;
...
/**
 * _getTok - gets the nth element of the buffer, separated by space
 * @n: element to get
 * @stat: stat array
 * Return: an array of the nth element or null if failed
 */
